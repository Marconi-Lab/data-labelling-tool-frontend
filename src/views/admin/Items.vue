<template>
  <div>
    <b-breadcrumb class="mb-0" :items="items"></b-breadcrumb>
     <b-nav style="position: absolute; z-index: 2222; right:0; top: 3.3em;">
      <b-nav-item active
        ><b-btn variant="info" v-b-modal.modal-center> images <b-icon icon="plus" style="float: right"></b-icon
      ></b-btn></b-nav-item>
    </b-nav>
     <vue-good-table
      :columns="fields"
    ...
    >
        <template slot="table-row" slot-scope="props">
            <span v-if="props.column.field=='name'">
                <router-link class="text-info" :to="`/admin/item/${props.column._id}`">{{props.row.name}}</router-link>
            </span>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [{"field": "name", "label":"Name"}, { "field": "labelled", "label":"Labelled"}, {"field":"_id", "label":"Delete"}],
      data: [],
      currentPage: 1,
      datasets,
    };
